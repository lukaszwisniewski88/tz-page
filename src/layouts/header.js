...
const takeUp = css`${tw`z-10`}`
const wrapperStyle = css`${tw`flex flex-col items-center relative p-4`}`
...
const Header = ({ siteTitle }) => (
  <header css={wrapperStyle}>
    <StaticImage alt="background" css={tw`absolute top-0 left-0 w-full h-full z-0` } objectFit="cover" objectPosition="50% 0%" quality="90" src="../images/bg_hero.png"/>
    <div css={takeUp}>
        <Logo/>
    </div>
    ...
    <nav tw="flex flex-col space-y-4 justify-center items-center z-10 text-center p-4">
      ...
      <MenuButton primary href='#Kontakt'>kontakt</MenuButton>
    </nav>
  </header>
)
